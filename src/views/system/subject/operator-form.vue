...
import { addSubjectsSchoolOperator, editSubjectsSchoolOperator } from '@/api/subjects/subject.js';
...
const contrastData = {
  college: {
    ...
    rules: {
      uid: [{ required: true, whitespace: true, message: '填写主体名称' }],
    ...
    const handleSubmit = e => {
      ...
      validate()
        .then(() => {
          ...
          if (props.id === '-1') {
            addSubjectsSchoolOperator(data)
              .then(res => {
                if (res.code === 200) {
                  loading.value = false;
                  emit('ok');
            ...
            editSubjectsSchoolOperator(props.id, data)
              .then(res => {
                if (res.code === 200) {
                  loading.value = false;
                  emit('ok');
