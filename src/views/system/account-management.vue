        ...
        <a-card :body-style="{ padding: 0 }" :bordered="false" ref="elRef">
          ...
          >
            ...
            <template #action="{ record }">
              <a-button
                type="link"
                @click="activeChangeClick(record, 0, '停用')"
              ...
              <a-button
                type="link"
                @click="activeChangeClick(record, 1, '启用')"
...
import {
  getSubjectsOperatorList,
  changeOperatorActive,
  deleteOperator,
} from '@/api/subjects/subject.js';
...
export default defineComponent({
  ...
  setup() {
    ...
    const { stripe, reload, setPageInfo, context: state } = useFetchData(getSubjectsOperatorList, {
      current: 1,
      pageSize: 100,
      tableSize: 'middle',
      stripe: false,
    ...
    // 启用 停用
    const activeChangeClick = (record, active, txt) => {
      Modal.confirm({
        title: `确定要${txt}吗？`,
        onOk() {
          changeOperatorActive(record.id, active)
            .then(() => {
              reload();
            })
            .catch(() => {});
